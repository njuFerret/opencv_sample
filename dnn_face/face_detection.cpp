...
bool FaceDetection::Finalize() { return true; }

bool FaceDetection::Process(const cv::Mat &image_input, std::vector<cv::Rect> &bbox_list,
...
void FaceDetection::GeneratePriors(const cv::Size &model_input_size) {
  ...
  std::pair<int32_t, int32_t> feature_map_2th = {(model_input_size.height + 1) / 2 / 2,
                                                 (model_input_size.width + 1) / 2 / 2};
...
void FaceDetection::PreProcess(const cv::Mat &image_input, cv::Mat &blob_input) {
  ...
  //  blob_input = cv::dnn::dnn4_v20230620::blobFromImage(blob_input);
  blob_input = cv::dnn::blobFromImage(blob_input);
  // WARNING 卧槽，是size，不是size()，尼玛，居然没有()
  std::cout << blob_input.size << std::endl;
}

void FaceDetection::Inference(const cv::Mat &blob_input, const std::vector<cv::String> output_name_list,
                              std::vector<cv::Mat> &output_mat_list) {
  net_.setInput(blob_input);
  net_.forward(output_mat_list, output_name_list);
}

void FaceDetection::PostProcess(const cv::Mat &mat_loc, const cv::Mat &mat_conf, const cv::Mat &mat_iou,
                                const cv::Size image_size, std::vector<cv::Rect> &bbox_list,
                                std::vector<Landmark> &landmark_list) {
  /* Get score list */
  ...
  for (int32_t row = 0; row < mat_loc.rows; row++) {
    ...
    bbox_all_list.push_back({static_cast<int32_t>((cx - w / 2) * image_size.width),
                             static_cast<int32_t>((cy - h / 2) * image_size.height),
                             static_cast<int32_t>(w * image_size.width), static_cast<int32_t>(h * image_size.height)});
  }
  ...
  for (int32_t row : indices) {
    ...
    for (int32_t landmark_index = 0; landmark_index < static_cast<int32_t>(landmark.size()); landmark_index++) {
      ...
      p.y =
          static_cast<int32_t>((prior_list_[row][1] + y * variance_list[0] * prior_list_[row][3]) * image_size.height);
    }
