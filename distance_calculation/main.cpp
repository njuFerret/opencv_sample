...
static void loop_main(const cv::Mat &image_org) {
  ...
  if (!image_org.empty()) {
    ...
    cv::projectPoints(original_object_point_list, camera.rvec, camera.tvec, camera.K, camera.dist_coeff,
                      image_point_list);
  ...
  cv::line(image, cv::Point(0, camera.EstimateVanishmentY()), cv::Point(image.cols, camera.EstimateVanishmentY()),
           cv::Scalar(0, 0, 0), 1);
...
static void TreatKeyInputMain(int32_t key) {
  ...
  key &= 0xFF;
  switch (key) {
  case 'w': camera.MoveCameraPos(0, 0, kIncPosPerFrame, false); break;
  case 'W': camera.MoveCameraPos(0, 0, kIncPosPerFrame, true); break;
  case 's': camera.MoveCameraPos(0, 0, -kIncPosPerFrame, false); break;
  case 'S': camera.MoveCameraPos(0, 0, -kIncPosPerFrame, true); break;
  case 'a': camera.MoveCameraPos(-kIncPosPerFrame, 0, 0, false); break;
  case 'A': camera.MoveCameraPos(-kIncPosPerFrame, 0, 0, true); break;
  case 'd': camera.MoveCameraPos(kIncPosPerFrame, 0, 0, false); break;
  case 'D': camera.MoveCameraPos(kIncPosPerFrame, 0, 0, true); break;
  case 'z': camera.MoveCameraPos(0, -kIncPosPerFrame, 0, false); break;
  case 'Z': camera.MoveCameraPos(0, -kIncPosPerFrame, 0, true); break;
  case 'x': camera.MoveCameraPos(0, kIncPosPerFrame, 0, false); break;
  case 'X': camera.MoveCameraPos(0, kIncPosPerFrame, 0, true); break;
  case 'q': camera.RotateCameraAngle(0, 0, 2.0f); break;
  case 'e': camera.RotateCameraAngle(0, 0, -2.0f); break;
  }
}
