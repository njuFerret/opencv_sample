...
class CameraModel {
   ...
   * s[x, y, 1] = K * [R t] * [Mw, 1]
   *   K: 相机的内部参数
   *   R: 相机在世界坐标系中的旋转矩阵（相机姿态）
   *   t: 相机坐标系中相机位置（Oc）到世界坐标系原点（Ow）的向量 = Ow - Oc = -RT（T = 从Ow到Oc的向量在世界坐标系中）
   *   Mw: 物体在世界坐标系中的坐标（Xw, Yw, Zw）
   *
   * s[x, y, 1] = K * Mc
   *   Mc: 物体在相机坐标系中的坐标（Xc, Yc, Zc） = R t * [Mw, 1]
   *
   * 推理: Mc = R(Mw - T) = RMw - RT = RMw + t   (t = -RT)
   *
   * 注意1: t是相机坐标系中的向量。因此，如果R发生了变化，则需要重新计算t。
   *
   * 注意2: 坐标系为右手坐标系。X+ = 右侧，Y+ = 向下，Z+ = 向远处（例如，如果物体在相机上方，Yc为负值）
   ***/
...
public:
  ...
  void ConvertWorld2Image(const std::vector<cv::Point3f> &object_point_list,
                          std::vector<cv::Point2f> &image_point_list) {
    /*** Mw -> Image ***/
    ...
    cv::Mat Rt =
        (cv::Mat_<float>(3, 4) << R.at<float>(0), R.at<float>(1), R.at<float>(2), this->tx(), R.at<float>(3),
         R.at<float>(4), R.at<float>(5), this->ty(), R.at<float>(6), R.at<float>(7), R.at<float>(8), this->tz());
...
#pragma omp parallel for
  ...
  void ConvertWorld2Camera(const std::vector<cv::Point3f> &object_point_in_world_list,
...
#pragma omp parallel for
    ...
    for (int32_t i = 0; i < object_point_in_world_list.size(); i++) {
      ...
      cv::Mat Mw =
          (cv::Mat_<float>(4, 1) << object_point_in_world.x, object_point_in_world.y, object_point_in_world.z, 1);
      cv::Mat Mc = Rt * Mw;
  ...
  void ConvertCamera2World(const std::vector<cv::Point3f> &object_point_in_camera_list,
                           std::vector<cv::Point3f> &object_point_in_world_list) {
    /*** Mc -> Mw ***/
    /* Mc = [R t] * [Mw, 1] */
...
#pragma omp parallel for
    ...
    for (int32_t i = 0; i < object_point_in_camera_list.size(); i++) {
      ...
      cv::Mat Mc =
          (cv::Mat_<float>(3, 1) << object_point_in_camera.x, object_point_in_camera.y, object_point_in_camera.z);
  ...
  void ConvertImage2GroundPlane(const std::vector<cv::Point2f> &image_point_list,
    ...
    if (this->dist_coeff.empty() || this->dist_coeff.at<float>(0) == 0) {
      ...
      cv::undistortPoints(image_point_list, image_point_undistort, this->K, this->dist_coeff,
                          this->K); /* don't use K_new */
    }
  ...
  void ConvertImage2Camera(std::vector<cv::Point2f> &image_point_list, const std::vector<float> &z_list,
                           std::vector<cv::Point3f> &object_point_list) {
    /*** Image -> Mc ***/
    if (image_point_list.size() == 0) {
...
#pragma omp parallel for
  ...
  void ConvertImage2World(std::vector<cv::Point2f> &image_point_list, const std::vector<float> &z_list,
                          std::vector<cv::Point3f> &object_point_list) {
    /*** Image -> Mw ***/
    std::vector<cv::Point3f> object_point_in_camera_list;
