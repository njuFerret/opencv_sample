...
void CommonHelper::DrawText(cv::Mat &mat, const std::string &text, cv::Point pos, double font_scale, int32_t thickness,
                            cv::Scalar color_front, cv::Scalar color_back, bool is_text_on_rect) {
  int32_t baseline = 0;
...
void CommonHelper::CropResizeCvt(const cv::Mat &org, cv::Mat &dst, int32_t &crop_x, int32_t &crop_y, int32_t &crop_w,
                                 int32_t &crop_h, bool is_rgb, int32_t crop_type, bool resize_by_linear) {
...
std::string CommonHelper::CreateGStreamerPipeline(int capture_width, int capture_height, int display_width,
  ...
  return "nvarguscamerasrc ! video/x-raw(memory:NVMM), width=(int)" + std::to_string(capture_width) + ", height=(int)" +
         ...
         ", format=(string)BGRx ! videoconvert ! video/x-raw, format=(string)BGR ! appsink max-buffers=1 drop=True";
}

bool CommonHelper::FindSourceImage(const std::string &input_name, cv::VideoCapture &cap, int32_t width,
                                   int32_t height) {
  if (input_name.find(".mp4") != std::string::npos || input_name.find(".avi") != std::string::npos ||
      input_name.find(".webm") != std::string::npos) {
    cap = cv::VideoCapture(input_name);
    if (!cap.isOpened()) {
      printf("Invalid input source: %s\n", input_name.c_str());
      return false;
    }
  } else if (input_name.find(".jpg") != std::string::npos || input_name.find(".png") != std::string::npos ||
             input_name.find(".bmp") != std::string::npos) {
...
bool CommonHelper::InputKeyCommand(cv::VideoCapture &cap) {
  ...
  do {
    ...
    case 'q':
      ...
      ret_to_quit = true;
      break;
    case 'p': is_pause = !is_pause; break;
    case '>':
      if (is_pause) {
...
void CommonHelper::drawTextCn(cv::Mat &mat, const std::string &text, const cv::Point &position, const cv::Scalar &color,
  ...
  ft2->putText(mat, text, position, fontSize, color, -1, cv::LINE_AA, true);
}
