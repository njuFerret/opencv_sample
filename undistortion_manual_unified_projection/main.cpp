...
static void CreateUndistortMap(cv::Size undist_image_size, float f_undist, float xi, float u0_undist, float v0_undist,
                               float f_dist, float u0_dist, float v0_dist, cv::Mat &mapx, cv::Mat &mapy);
...
static void loop_main(const cv::Mat &image_org) {
  ...
  if (update_camera_parameter) {
    CreateUndistortMap(undist_image_size, f_undist, camera_parameter.xi, u0_undist, v0_undist, f_dist, u0_dist, v0_dist,
                       mapx, mapy);
    ...
    cv::resize(image_undistorted, image_undistorted, cv::Size(), 1.0 / new_image_size_scale,
               1.0 / new_image_size_scale);
...
static void loop_param() {
  ...
  cvui::beginColumn(mat, 10, 10, -1, -1, 10);
  {
    ...
    MAKE_GUI_SETTING_FLOAT(new_image_size_scale, "Scale", 1.0f, "%.0f", 0.0f, 10.0f);

    cvui::text("Camera Parameter (Unified projection model)");
    MAKE_GUI_SETTING_FLOAT(camera_parameter.focal_length, "Focal Length", 10.0f, "%.0f", 0.0f, 1000.0f);
    MAKE_GUI_SETTING_FLOAT(camera_parameter.xi, "xi", 0.001f, "%.03f", 0.01f, 1.2f);
...
static void CreateUndistortMap(cv::Size undist_image_size, float f_undist, float xi, float u0_undist, float v0_undist,
  ...
  for (int32_t y = 0; y < undist_image_size.height; y++) {
    for (int32_t x = 0; x < undist_image_size.width; x++) {
      Alpha_Cam.at<float>(y, x) =
          1 / sqrtf(X_Cam.at<float>(y, x) * X_Cam.at<float>(y, x) + Y_Cam.at<float>(y, x) * Y_Cam.at<float>(y, x) +
  ...
  for (int32_t y = 0; y < undist_image_size.height; y++) {
    for (int32_t x = 0; x < undist_image_size.width; x++) {
      den.at<float>(y, x) =
          xi * sqrtf(X_Sph.at<float>(y, x) * X_Sph.at<float>(y, x) + Y_Sph.at<float>(y, x) * Y_Sph.at<float>(y, x) +
