...
bool DepthEngine::Finalize() { return true; }
...
bool DepthEngine::NormalizeScaleShift(const cv::Mat &mat_depth, cv::Mat &mat_depth_normalized, float scale,
                                      float shift) {
  /***
...
void DepthEngine::PreProcess(const cv::Mat &image_input, cv::Mat &blob_input) {
  ...
  cv::subtract(image_normalize, cv::Scalar(cv::Vec<float, 3>(kMeanList[0], kMeanList[1], kMeanList[2])),
               image_normalize);
...
void DepthEngine::Inference(const cv::Mat &blob_input, const std::vector<cv::String> output_name_list,
                            std::vector<cv::Mat> &output_mat_list) {
