...
void ResetCameraPose() {
  camera_real.SetExtrinsic({0.0f, 0.0f, 0.0f}, /* rvec [deg] */
                           {0.0f, -1.0f, 0.0f},
                           true); /* tvec (Oc - Ow in world coordinate. X+= Right, Y+ = down, Z+ = far) */

  camera_top.SetExtrinsic({90.0f, 0.0f, 0.0f}, /* rvec [deg] */
                          {0.0f, -5.0f, 7.0f},
                          true); /* tvec (Oc - Ow in world coordinate. X+= Right, Y+ = down, Z+ = far) */
}
...
static void loop_main(const cv::Mat &image_org) {
  ...
  std::vector<cv::Point3f> object_point_list = {
      /* Target area (possible road area) */
      {-1.0f, 0, 10.0f},
      {1.0f, 0, 10.0f},
      {-1.0f, 0, 3.0f},
      {1.0f, 0, 3.0f},
  ...
  cv::projectPoints(object_point_list, camera_real.rvec, camera_real.tvec, camera_real.K, camera_real.dist_coeff,
                    image_point_real_list);

  /* Convert to image points (2D) using the top view camera (virtual camera) */
  std::vector<cv::Point2f> image_point_top_list;
  cv::projectPoints(object_point_list, camera_top.rvec, camera_top.tvec, camera_top.K, camera_top.dist_coeff,
                    image_point_top_list);
  ...
  cv::warpPerspective(image_org, mat_output, mat_transform, mat_output.size(), cv::INTER_LINEAR,
...
static void loop_param() {
  ...
  cvui::beginColumn(mat, 10, 10, -1, -1, 2);
  {
    ...
    MAKE_GUI_SETTING_FLOAT(camera_real.fx(), "Focal Length", 10.0f, "%.0f", 0.0f, 1000.0f);
    ...
    MAKE_GUI_SETTING_FLOAT(pitch_deg, "Pitch", 1.0f, "%.0f", -180.0f, 180.0f);
    float yaw_deg = Rad2Deg(camera_top.ry());
    MAKE_GUI_SETTING_FLOAT(yaw_deg, "Yaw", 1.0f, "%.0f", -180.0f, 180.0f);
    ...
    MAKE_GUI_SETTING_FLOAT(y, "Height", 0.5f, "%.1f", 0.0f, -5.0f);
...
static void TreatKeyInputMain(int32_t key) {
  ...
  key &= 0xFF;
  switch (key) {
  case 'w': camera_top.MoveCameraPos(0, 0, kIncPosPerFrame, false); break;
  case 'W': camera_top.MoveCameraPos(0, 0, kIncPosPerFrame, true); break;
  case 's': camera_top.MoveCameraPos(0, 0, -kIncPosPerFrame, false); break;
  case 'S': camera_top.MoveCameraPos(0, 0, -kIncPosPerFrame, true); break;
  case 'a': camera_top.MoveCameraPos(-kIncPosPerFrame, 0, 0, false); break;
  case 'A': camera_top.MoveCameraPos(-kIncPosPerFrame, 0, 0, true); break;
  case 'd': camera_top.MoveCameraPos(kIncPosPerFrame, 0, 0, false); break;
  case 'D': camera_top.MoveCameraPos(kIncPosPerFrame, 0, 0, true); break;
  case 'z': camera_top.MoveCameraPos(0, -kIncPosPerFrame, 0, false); break;
  case 'Z': camera_top.MoveCameraPos(0, -kIncPosPerFrame, 0, true); break;
  case 'x': camera_top.MoveCameraPos(0, kIncPosPerFrame, 0, false); break;
  case 'X': camera_top.MoveCameraPos(0, kIncPosPerFrame, 0, true); break;
  case 'q': camera_top.RotateCameraAngle(0, 0, 2.0f); break;
  case 'e': camera_top.RotateCameraAngle(0, 0, -2.0f); break;
  }
}
