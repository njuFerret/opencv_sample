...
int main(int argc, char *argv[]) {
  ...
  static const std::vector<std::string> image_path_list = {
      RESOURCE_DIR "/chessboard/left01.jpg", RESOURCE_DIR "/chessboard/left02.jpg",
      RESOURCE_DIR "/chessboard/left03.jpg", RESOURCE_DIR "/chessboard/left04.jpg",
      RESOURCE_DIR "/chessboard/left05.jpg", RESOURCE_DIR "/chessboard/left06.jpg",
      RESOURCE_DIR "/chessboard/left07.jpg", RESOURCE_DIR "/chessboard/left08.jpg",
      RESOURCE_DIR "/chessboard/left09.jpg", RESOURCE_DIR "/chessboard/left11.jpg",
      RESOURCE_DIR "/chessboard/left12.jpg", RESOURCE_DIR "/chessboard/left13.jpg",
  ...
  for (int32_t i = 0; i < kHorizonalCrossCount * kVerticalCrossCount; i++) {
    object_point.push_back(
        cv::Point3f(static_cast<float>(i / kHorizonalCrossCount), static_cast<float>(i % kHorizonalCrossCount), 0.0f));
  }

  std::vector<std::vector<cv::Point3f>> object_point_list;
  std::vector<std::vector<cv::Point2f>> image_point_list;
  for (int32_t i = 0; i < (int)image_path_list.size(); i++) {
    cv::Mat image_chessboard = cv::imread(image_path_list[i]);
    if (image_chessboard.channels() == 3)
      cv::cvtColor(image_chessboard, image_chessboard, cv::COLOR_BGR2GRAY);
    std::vector<cv::Point2f> image_point;
    bool found = findChessboardCorners(image_chessboard, chessboard_pattern, image_point,
                                       cv::CALIB_CB_ADAPTIVE_THRESH | cv::CALIB_CB_NORMALIZE_IMAGE);
    //    object_point_list.reserve(object_point_list.size() + 2);
    //    image_point_list.reserve(image_point_list.size() + 2);
    if (found) {
      printf("corner found\n");
